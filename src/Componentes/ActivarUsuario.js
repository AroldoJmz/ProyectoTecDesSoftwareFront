...
export default function ActivarUsuario({user, reloadList}){
    ...
    return(
        user.activo === true 
        ? 
            <button onClick={editUserStatus} className="btn btn-outline-success" type="submit"><FaUserCheck/></button>  
        :
            <button onClick={editUserStatus} className="btn btn-outline-danger" type="submit"><FaUserXmark/></button>    
    )
}